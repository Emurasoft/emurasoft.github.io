...
| eeFileDialogFileMustExist | 사용자가파일 이름 입력 칸에 존재하는 파일의 이름만을 입력할 수 있도록 지정합니다.<br> 이 플래그가 지정되고 사용자가 유효하지 않은 이름을 입력하는 경우, 대화 상자 프로시저가 메시지 박스에 경고문을 표시합니다.<br> 이 플래그가 지정된 경우, eeFileDialogPathMustExist 플래그도 사용됩니다. |
...
| eeFileDialogNoReadOnlyReturn | 반환된 파일이읽기 전용 체크 박스가 선택되어 있지 않고,<br> 쓰기 보호 디렉토리가 되지 않게 지정합니다. |
...
사용자가취소 버튼을 선택한 경우 빈 문자열을 반환합니다.
