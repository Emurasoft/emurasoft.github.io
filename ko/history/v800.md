...
-복사 명령은 이제 텍스트가 선택되어지지 않았을 때에도 현재 문서줄 전체를 복사합니다. 이전 동작은복사 선택 명령을 이용해서 선택되어질 수 있습니다.
-잘라내기 명령은 이제 텍스트가 선택되지 않았을 때에도 현재 문서줄 전체를 잘라냅니다. 이전 동작은잘라내기 선택 명령을 이용해서 선택되어질 수 있습니다.
...
- 새로운다음으로 이동 과이전으로 이동 명령은 출력 바의 결과값 사이에서 이동하는데 사용되어 질 수 있습니다.
- CTRL에서 마우스 휠을 사용하면 글자 크기를 늘리거나 줄일 수 있습니다.
- 오른쪽 클릭에서 마우스 휠은 다음 혹은 이전 문서로 이동합니다.

## 새로운 플러그 인 기능

- 플러그 인프로젝트 는 솔루션 템플릿, 도구 모음, 매개변수 정보의 도구 설명 등과 함께 개선되었습니다. 또한 이것을 이용해서 Visual Studio 솔루션 파일들을 읽기 전용으로 열어 볼 수 있습니다.
-워드 자동완성 플러그 인은 이제 최근 검색된 문자열을 완성시킵니다.

## 새로운 옵션

- [바꾸기 대화 상자](../dlg/replace/index) 에 있는열려있는 모든 문서 검색 체크 박스.
- 구성 속성의 [스크롤 탭](../dlg/properties/scroll/index) 안의점선 라인 체크 박스.
- 구성 속성의 [표시 탭](../dlg/properties/marks/index) 안의들여쓰기 가이드 보여주기 체크 박스.
- 사용자 지정 대화 상자의 [상태 탭](../dlg/customize/status/index) 안의돌아가기 방법 체크 박스.
-사용자 지정 대화 상자의 [편집 탭](../dlg/customize/edit/index).
- 사용자 지정 대화 상자의 [고급 탭](../dlg/customize/advanced/index) 에 있는시스템 임시 폴더 이용 체크 박스와임시 폴더 텍스트 상자.
-사용자 지정 대화 상자의 [표시 탭](../dlg/customize/marks/index).
- 사용자 지정 대화 상자의 [윈도우 탭](../dlg/customize/window/index) 에 있는마우스로 활성화시 커서 위치 지정 체크 박스.
- [도구 모음 대화상자](../dlg/toolbars/index) 의기본 도구 모음에서 매크로 표시 체크 박스
-파일 놓기 이벤트는 [이벤트 선택 대화 상자](../dlg/macro_customize/my_macros/select_events/index) 의이벤트 선택 목록 박스에 추가되었습니다.
- [파일 내 찾기 대화 상자](../dlg/find_in_files/index) 의출력 모음 사용 체크 박스.
...
속성이 추가되었습니다.
- [ScrollProp 개체](../macro/scroll_prop/index) 에DottedLine 속성이 추가되었습니다.
