...
현재 커서 위치에 기존의 문자열을 덮어써서 ANSI 문자열을 삽입합니다. 이 인라인 함수를 사용하거나
...
> 대상 반환 유형 (CR 만, LF 만 또는 CR과 LF)이 유지되도록 지정합니다. 이 매개 변수가 생략된경우, EmEditor는 szString 매개 변수에서 지정된 반환 유형을 유지합니다.
