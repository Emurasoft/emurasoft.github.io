...
EmEditor에서 사용되는 정보 매개 변수 중 하나의 값을 설정하거나 검색합니다. 이 메시지를 명시적으로 보내거나
...
| EI\_GET\_FULL\_TITLEA | ANSI 문자열 내 문서의 전체 제목을 검색합니다. | (LPSTR)szTitle<br> 문자열을 검색하기 위해 버퍼를 지정합니다.<br> 버퍼는 종료된 NULL문자를 포함하는 MAX\_PATH 단어만큼이어야 합니다. | 사용되지 않습니다. |
| EI\_GET\_FULL\_TITLEW | 유니코드 문자열 내 문서의 전체 제목을 검색합니다. | (LPWSTR)szTitle<br> 문자열을 검색하기 위해 버퍼를 지정합니다.<br> 버퍼는 종료된 NULL문자를 포함하는 MAX\_PATH 단어만큼이어야 합니다. | 사용되지 않습니다. |
| EI\_GET\_SHORT\_TITLEA | ANSI 문자열 내 문서의 짧은 제목을 검색합니다. | (LPSTR)szTitle<br> 문자열을 검색하기 위해 버퍼를 지정합니다.<br> 버퍼는 종료된 NULL문자를 포함하는 MAX\_PATH 단어만큼이어야 합니다. | 사용되지 않습니다. |
| EI\_GET\_SHORT\_TITLEW | 유니코드 문자열 내 문서의 짧은 제목을 검색합니다. | (LPWSTR)szTitle<br> 문자열을 검색하기 위해 버퍼를 지정합니다.<br> 버퍼는 종료된 NULL문자를 포함하는 MAX\_PATH 단어만큼이어야 합니다. | 사용되지 않습니다. |
| EI\_GET\_SAVE\_AS\_TITLEA | ANSI 문자열 내 수정을 나타내는 별표(\*)를 제외한 문서의 전체 제목을 검색합니다. | (LPSTR)szTitle<br> 문자열을 검색하기 위해 버퍼를 지정합니다.<br> 버퍼는 종료된 NULL문자를 포함하는 MAX\_PATH 단어만큼이어야 합니다. | 사용되지 않습니다. |
| EI\_GET\_SAVE\_AS\_TITLEW | 유니코드 문자열 내 수정을 나타내는 별표(\*)를 제외한 문서의 전체 제목을 검색합니다. | (LPWSTR)szTitle<br> 문자열을 검색하기 위해 버퍼를 지정합니다.<br> 버퍼는 종료된 NULL문자를 포함하는 MAX\_PATH 단어만큼이어야 합니다. | 사용되지 않습니다. |
...
| EI\_SAVE\_DOC | 문서를 저장합니다. 문서의 제목이 정해지지 않은 경우다른 이름으로 저장 대화 상자가 나타납니다. | 사용되지 않습니다. | (BOOL)bSuccess<br> 성공하면 TRUE를 실패하는 경우 FALSE를 반환합니다. 문서에 이름이 지정되지 않았을 때다른 이름으로 저장 대화 상자에서 취소를 누르면 FALSE가 반환됩니다. |
| EI\_GET\_CURRENT\_FOLDER | 현재 작업중인 폴더를 검색합니다. | (LPWSTR)szDir<br> 문자열을 검색하기 위해 버퍼를 지정합니다.<br> 버퍼는 종료된 NULL문자를 포함하는 MAX\_PATH 단어만큼이어야 합니다. | 사용되지 않습니다. |
...
대상 문서의 인덱스를 지정합니다.1로 시작하는 인덱스는 wParam의 상위 단어에 지정되어야 합니다.
wParam의 상위 단어에 0이 지정되면, 현재 활성화 중인 문서가 대상으로 지정됩니다.
이 매개 변수는 nCmd에 의해 결정되지 않을 수 있습니다. 이런 경우에는, wParam의 상위 단어가 반드시 0이 됩니다.
...
EmEditor 버전 3.00 이상에서만 지원됩니다.
하지만, iDoc 매개 변수는 EmEditor 버전 5 이상에서만 지원됩니다.
