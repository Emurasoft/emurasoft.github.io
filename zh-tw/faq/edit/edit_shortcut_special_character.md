...
然后把這個檔案儲存為InsertA.jsee。在「巨集」 下拉單中點擊「選擇...」，并打開 InsertA.jsee。當運行這個巨集時，EmEditor 便會自動插入這個字元。您只需要打開「工具」下的「組態屬性」，到「鍵盤」頁面上，在「類別」下拉清單中選擇「我的巨集」，并自訂一個快速鍵給這個巨集，設定完成后，您就能用您自訂的快速鍵來插入這個特殊字元了。
