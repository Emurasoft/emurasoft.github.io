...
在 Windows Vista 或更新的系統上操作時，安裝程式一定要在管理員特權上運行，來避免「使用者帳戶控制」提示。例如，如果您要在命令提示符上運行安裝程式，您需要在按 SHIFT 鍵的同時右擊命令提示符圖示來打開命令提示符視窗，然後選擇作為管理員運行 程式。此外，您必須指定MSIINSTALLPERUSER="" 才能為所有使用者安裝 EmEditor v20.9 或更高版本。要僅為目前的使用者安裝 EmEditor v20.8 或更早版本，您必須指定MSIINSTALLPERUSER=1。
