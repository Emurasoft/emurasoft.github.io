...
如果勾選此項，EmEditor 只會在您按下快捷鍵 Ctrl+Space 時顯示建議，這有助於降低 AI 呼叫的頻率。
...
如果勾選此項，溫度參數將會啟用，用於調整隨機性。並非所有 AI 模型都支援此參數。

## 「在下列任一字元之後不顯示建議」核取方塊

如果勾選此項，當游標位於下列任一字元之後時，EmEditor 將不會顯示建議。
...
使用此滑桿來調整延遲時間。較長的延遲時間會讓建議出現得更慢。

## 「額外指示」文字方塊

指定額外指示。例如：「使用 JavaScript」或「以禮貌的語氣撰寫」。
