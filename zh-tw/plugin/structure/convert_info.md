...
用於 [EE\_CONVERT\_EX](../message/ee_convert_ex) 消息。
...
你可以指定以下值的組合。必須指定 SORT\_ENABLED 以對分割字串進行排序，可以與其他標志合用來指定排序行為。必須指定 SORT\_DELETE\_DUPLICATE 以刪除重複的分割字串。
