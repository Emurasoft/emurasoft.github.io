...
指定要將目前的文檔轉換為 CSV 格式的索引。0 表示固定寬度的欄格式（非 CSV）。1 表示**「自訂」**對話方塊中 **「CSV 格式」**頁面上的第一個定義的格式（預設情況下是「逗號分隔」）。
...
| CSV_TRUNCATE_UNFIT | 截斷字串如果字串長於欄寬。 |
| CSV_PROMPT_INVALID | 發出警告如果偵測到無效格式。 |
