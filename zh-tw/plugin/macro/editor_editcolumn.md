...
移動，複製，刪除，或合併目前的 CSV 文檔中的指定欄。你能直接用該內嵌函式或明確地發送 [EE\_EDIT\_COLUMN](../message/ee_edit_column) 消息。
...
> | COLUMN\_COPY | 把從 _iColumn1_ 到 _iColumn2_ 的欄複製到 _iColumnTo_ 的欄之前。 |
> | COLUMN\_CONCAT | 串連從 _iColumn1_ 到 _iColumn2_ 的欄，可以選擇使用 _pszInsert_ 作為分隔符。 |
...
> 串連或分割欄時，將一個字串指定為分隔符。僅當指定了 COLUMN\_CONCAT 時，才使用此欄位。

_nCombineFlags_

> 您可以指定以下值的組合。必須指定 SORT\_ENABLED 對字串進行排序，並與其他標志結合以指定排序行為。必須指定 SORT\_DELETE\_DUPLICATE 才能刪除重複的字串。只有在 _nFlags_ 參數中指定了 COLUMN\_CONCAT 時，才能使用此參數。此參數可以省略。
>
> | 值 | 含義 |
> | --- | --- |
