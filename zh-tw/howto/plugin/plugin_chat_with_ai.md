...
- 在聊天面板底部的輸入框中輸入您的提示詞。
- 點選 **+ 檔案** 以上傳檔案。
  - 此功能僅適用於 Google 和 OpenAI。檔案搜尋會產生額外費用。
  - 您也可以點選**瀏覽**，透過檔案瀏覽器對話方塊選取支援的檔案。
  - 上傳檔案後，「與 AI 聊天」會同時建立檔案與向量儲存（vector store）資源。之後即可在聊天對話中引用該檔案。
  - 已上傳的檔案會列在輸入區域中。點選檔案可移除，或按一下滑鼠右鍵以取得更多選項。
  - 引用已上傳的檔案通常比直接將檔案內容複製貼上到聊天中消耗更少的 token。
- 啟用**工具**以使用工具呼叫（tool-calling）功能。AI 可使用這些 EmEditor 工具來讀取或修改您的文件。
  - 將滑鼠游標移到**工具**上，並選取**設定**以檢視與設定可用的工具。
  - 使用工具將產生額外的 token 費用。
