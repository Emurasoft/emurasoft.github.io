...
### Anthropic
1. 登入 Claude Console，並前往 [API 金鑰頁面](https://platform.claude.com/settings/keys)。
2. 點選 **建立金鑰**，並為金鑰新增名稱。
3. 複製 API 金鑰。
...
- 在聊天面板底部的輸入框中輸入您的提示詞。
- 按一下 **上傳檔案** 按鈕以上傳檔案。  
  - 此功能僅適用於 Google 與 OpenAI。檔案搜尋會產生額外費用。  
  - 按一下 **瀏覽**，透過檔案瀏覽對話方塊選取支援的檔案。  
  - 上傳檔案後，「與 AI 聊天」會同時建立檔案與向量儲存資源。之後即可在聊天對話中引用該檔案。  
  - 已上傳的檔案會列在輸入欄位下方。按一下檔案可移除，或按滑鼠右鍵以取得更多選項。  
  - 引用已上傳的檔案通常比直接將檔案內容複製貼上到聊天中消耗更少的代幣（token）。  
- 按一下 **AI 模型名稱** 以變更每個聊天的設定。  
  - 此處所做的變更只會套用到目前的聊天工作階段。若要調整全域設定，請使用側邊欄中的 **設定** 按鈕。  
- 按一下 🔨（**工具**）按鈕以檢視工具呼叫的設定。這些 EmEditor 工具允許 AI 讀取或修改您的文件。  
  - 使用工具會產生額外的 token 費用。  
- 按一下 🔌（**MCP**）按鈕以開啟 MCP 連接器設定。MCP 連接器會在另一個章節中說明。
...
- 此對話框會顯示與您的帳號或組織相關聯的所有檔案與向量儲存庫。對於每個向量儲存庫，也會顯示其關聯的聊天以及目前狀態。
  - 已過期的向量儲存庫不會產生儲存費用。
- 您可以從您的帳號或組織中選取並刪除檔案或向量儲存庫。

## 匯入與匯出聊天記錄
- 您可以將聊天對話匯出到檔案。此檔案可用於匯入，以還原對話內容。
