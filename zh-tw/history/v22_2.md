...
- 添加了\\V 運算式到 [取代運算式語法](../howto/search/replacement_expression_syntax)，讓您能把V8 JavaScript 引擎 用於取代運算式。
- [CommitList 外掛程式](../howto/plugin/plugin_commit_list) 現在可以調用拉取、推送和擷取命令。
- 添加了能篩選記錄的功能到CommitList 中。
- 改進了崩潰報告。
- 更新了CSS HTML Validator Lite 到 v21.0。
...
- 添加了自動管理所有記憶體大小 核取方塊到自訂 對話方塊的 [進階 頁面](../dlg/customize/advanced/index) 上。
- 添加了拉取 和推送 按鈕以及擷取 功能表項目到 [CommitList 外掛程式](../howto/plugin/plugin_commit_list) 中。
- 添加了篩選記錄 對話方塊到 [CommitList](../howto/plugin/plugin_commit_list) 中。
- 添加了總是丟棄過長的復原信息來加速 選項到進度 對話方塊中（與自訂 對話方塊的編輯 頁面中同名的選項相同）。
...
- 添加了nFlags 參數到 [EE\_SET\_STATUSW 消息](../plugin/message/ee_set_statusw) 和 [Editor\_SetStatusW 內嵌函式](../plugin/macro/editor_setstatusw) 中。

## 新增巨集功能 <pro />

- 添加了eeMacroLangV8 和eeMacroSyncOnly 標志到Editor 對象的 [ExecuteMacro 方法](../macro/editor/editor_executemacro) 中。
