...
- 添加了僅打開與指定的打開並篩選 符合的行的功能。這讓你可以從一個非常大的檔案中只打開包含特定字串（如 ID 或名稱）的行，來最小化記憶體使用空間。該功能還能讓你編輯或排序指定的行，並將這些行儲存到新檔案。
...
- 當要關閉一個已修改的文檔時，出現的要儲存變更嗎？ 對話方塊中添加了儲存工作區 按鈕添加。點擊此按鈕將所有修改後的文檔儲存為帶復原信息的工作區，下次打開 EmEditor 時，所有修改過的文檔將被恢復並且所有復原信息會被保留。

## 新增選項

- 添加了篩選 下拉清單方塊，> 和X 按鈕，使用臨時檔案，大小寫須相符，使用規則運算式，使用逸出序列 核取方塊，預設 文字方塊到大檔案控制器 自訂工具條中。
- 添加了當打開並篩選是預設時提示 核取方塊到自訂 對話方塊的 [檔案 頁面](../dlg/customize/file/index) 上。
- 添加了排除資料夾 清單方塊，添加 和刪除 按鈕到自訂 對話方塊的 [歷史記錄 頁面](../dlg/customize/history/index) 上。
- 把排除網際網路快取資料夾 核取方塊從自訂 對話方塊的 [檔案 頁面](../dlg/customize/file/index) 上移動到了 [歷史記錄 頁面](../dlg/customize/history/index) 上。
- 把監視外部應用程式中的剪貼簿複製內容 和添加額外的資訊到復原/重做歷史記錄中 核取方塊從自訂 對話方塊的 [歷史記錄 頁面](../dlg/customize/history/index) 上移動到了 [編輯 頁面](../dlg/customize/edit/index) 上。
- 添加了打開並篩選的篩選顏色 到組態屬性對話方塊中 [顯示 頁面](../dlg/properties/display/index) 上的指定部分 清單方塊中。
- 添加了 [工作區 頁](../dlg/customize/workspace/index) 到自訂 對話方塊中。
- 把自動工作區 下拉清單方塊以及退出前不再詢問 核取方塊從自訂 對話方塊中的 [視窗 頁](../dlg/customize/window/index) 移動到 [工作區 頁](../dlg/customize/workspace/index) 上。
- 添加了儲存總是最上層顯示狀態 選項到自訂 對話方塊的 [視窗 頁面](../dlg/customize/window/index) 中。
- 添加了亮顯活動標籤 核取方塊以及色彩 按鈕到自訂對話方塊中的 [標籤 頁面](../dlg/customize/tab/index) 上。
...
- 添加了從不，並在完成時保留原文 選項到 [字詞自動完成 屬性](../howto/plugin/plugin_wordcomplete)比對準則 頁面上的大小寫需相符 下拉清單方塊中。

## 外掛程式 API 新增功能

- 添加了EI\_GET\_MEMORY\_SIZE 和EI\_SET\_MEMORY\_SIZE 命令到 [EE\_INFO 消息](../plugin/message/ee_info) 中。
- 添加了LFI\_USE\_TEMP\_FILE 和LFI\_DONT\_USE\_TEMP\_FILE 到 [LOAD\_FILE\_INFO\_EX 結構](../plugin/structure/load_file_info) 中。

## 新增巨集功能

- 添加了 [MemorySize 屬性](../macro/document/memory_size) 到Document 對象中。
- 添加了eeUseTempFile 和eeDontUseTempFile 標志到 [OpenFile 方法](../macro/editor/editor_openfile) 中。
