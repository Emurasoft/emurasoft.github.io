...
- 新增了添加引號到所有儲存格上 按鈕到CSV 轉換器 上。
- 在自訂 對話方塊的 [視窗 頁面](../dlg/customize/window/index) 上，添加了播放額外音效 核取方塊。
- 在自訂 對話方塊的進階 頁面上最大記憶體大小 的預設值變為 90%。
- 新增了「將焦點設在輸出列上」 核取方塊到 [外部工具屬性](../dlg/tools/properties/index) 對話方塊中。
- 廢棄了在自訂巨集 對話方塊中的 [「選項」 頁面](../dlg/macro_customize/options/index) 上的錄制游標活動和鍵盤輸入到其他應用程式中 核取方塊，錄制所有游標移動 核取方塊，以及在插入休眠模式前的空閑時間 文字方塊。
...
- 添加了 [filters 屬性](../macro/document/filters) 以及 [Untitled 屬性](../macro/document/untitled) 到Document 對象中。
- 添加了eeFindFileAndLine，eeFindFileNamesOnly，eeFindLineOnly，eeFindMatchedOnly，和eeFindOutput 標志到Selection 對象的 [Find 方法](../macro/selection/selection_find) 中。
