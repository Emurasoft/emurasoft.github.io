...
- 提高了各種排序 命令的速度。
- 檔案比較後添加了生成報告檔案的功能。 添加了eeCompare 組態和以垂直線 分隔的 CSV 格式來檢視比較報告。
- 將Python 語法更新到 Python 3.7.0b4。
...
- 添加了生成報告？ 頁到比較 精靈中。
- 添加了檔案修改日期 (&e) 和檔案修改時間 (&u) 巨集到組態屬性的 [列印 頁面](../dlg/properties/print/index) 上的頁眉/頁腳 文字方塊中。
- 添加了顏色 下拉清單方塊到組態屬性的 [亮顯 (1) 頁面](../dlg/properties/highlight1/index) 上，讓你能指定與比較相關的顏色。
- 添加了使用臨時檔案的最小大小 文字方塊以及目前使用臨時檔案 消息到大檔案控制器 自訂顯示條中。
- 添加了忽略空儲存格 核取方塊到 [刪除/把重複行設為書籤 對話方塊](../dlg/delete_duplicate_advanced/index) 中。
- 添加了比較 CSV 文檔儲存格中未加引號的字串 核取方塊到自訂 對話方塊中的 [排序 頁面](../dlg/customize/sort/index) 上。
...
- 添加了MANAGE\_DUPLICATES\_IGNORE\_EMPTY\_CELLS 標志到 [MANAGE\_DUPLICATES\_INFO 結構](../plugin/structure/manage_duplicates_info) 中。
- 添加了SORT\_UNQUOTE\_CELLS 標志到 [SORT\_INFO 結構](../plugin/structure/sort_info) 中。
...
- 添加了eeIgnoreEmptyCells 標志到Document 對象的 [DeleteDuplicates 方法](../macro/document/delete_duplicates) 中。
- 添加了eeSortUnquoteCells 標志到Document 對象的 [Sort 方法](../macro/document/sort) 中。
- 添加了 [Compare 方法](../macro/editor/compare) 到Editor 對象中。
