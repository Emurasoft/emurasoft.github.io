...
- 添加了同步 設定功能。預設設定不啟用此功能。
- 添加了/max 選項到 [命令列選項](../howto/file/file_commandline) 中。

## 新增選項

- 添加了 [同步 頁面](../dlg/customize/sync/index) 到自訂 對話方塊中。
- 添加了一個儲存格中的最大新行數 文字方塊到自訂 對話方塊中的 [CSV 頁面](../dlg/customize/csv/index) 上。
- 添加了每個檔案的最大記憶體大小 文字方塊到自訂 對話方塊中的 [進階 頁面](../dlg/customize/advanced/index) 上。
- 添加了App 主題 下列清單方塊到自訂 對話方塊的 [檢視 頁面](../dlg/customize/view/index) 上。
- 添加了停止如果符合數達到 文字方塊到 [多檔尋找](../dlg/find_in_files/index) 以及 [多檔取代](../dlg/replace_in_files/index) 對話方塊中。棄用了自訂 對話方塊中 [搜尋 頁面](../dlg/customize/search/index) 上的如使文檔保持開啟，可取代的最多檔案數目 文字方塊。
...
- 添加了nLimit 欄位到 [GREP\_INFO\_EX 結構](../plugin/structure/grep_info_ex) 中。

## 新增巨集功能

- 添加了nExFlags 和nLimit 參數到Editor 對象的 [FindInFiles 方法](../macro/editor/editor_findinfiles) 以及 [ReplaceInFiles 方法](../macro/editor/editor_replaceinfiles) 中。
