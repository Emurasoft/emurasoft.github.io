...
- 在篩選 工具列上新增了「負向」 按鈕，讓您能排除與篩選字串符合的行。
- 在CSV/排序 工具列上新增了按文字長度從短到長 和按文字長度從長到短 按鈕，讓您能按列中文字的長度排序文檔。
- 新的版本讓您能在 CSV 文檔中按特定的列篩選。另外您能指定多個列來進行篩選 (進階篩選)。

## 新增選項

- 在自訂 對話方塊中新增了將數字作為數量處理 核取方塊以及當按長度排序時，將全形字元作為 2 個字元排序 核取方塊。
- 添加了 [進階篩選 對話方塊](../dlg/advanced_filter/index)。

## 新增命令

-[負向 (篩選工具列)](../cmd/search/filterbar_negative)
- [按文字長度從短到長](../cmd/edit/sort_length_a)
- [按文字長度從長到短](../cmd/edit/sort_length_d)
