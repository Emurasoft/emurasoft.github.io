...
通常在 [Replace 方法](../selection/selection_replace)，與 [Find 方法](../selection/selection_find) 相同，不會終止執行一個巨集當沒有找到搜尋字串。但是，有一個例外。如果您用巨集 功能表下的 [使用臨時選項運行 命令](../../cmd/macros/macro_run_options)，并在快顯的 [巨集臨時選項 對話方塊](../../dlg/macro_temp_options/index) 中勾選了搜尋失敗即停止 核取方塊，那么巨集就會被終止當沒有找到搜尋字串時。詳見教程中的 [尋找一個字串](tutorial_find)。
