...
| eeFileDialogFileMustExist | 指定用戶只能輸入已存在檔案的檔案名在檔案名 輸入欄位。如果指定了該標志而用戶輸入了一個無效的名稱，對話方塊會顯示一條警告消息。如果指定了該標志，eeFileDialogPathMustExist 也會被用到。 |
