...
數字範圍用區間表示。方括號表示包含起始或結尾數字的端點，圓弧括號表示排除起始或結尾數字的端點。如果區間中的數字都不包含小數點，則該方法僅符合整數。當表示負數或正數無窮大時，可以省略最小或最大值。數字格式取決於 **自訂** 對話方塊中 [**排序** 頁面](../../dlg/customize/sort/index) 上 **區域設定** 下拉清單方塊中指定的區域。在某些語言環境（例如德語）中，逗號（'，'）用於表示小數點，在這種情況下，有必要在分隔符逗號之前和之後插入一個空格來避免歧義。
