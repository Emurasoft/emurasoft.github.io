...
- 用 **WebView2** 控件在 EmEditor 中添加了显示嵌入式**网页浏览器**的功能。可以在**自定义**对话框的新**网页浏览器**页面中自定义初始主页 URL。**网页浏览器**设计为与宏结合使用。编写宏使您能够将特定文本发送到网站。**onLoad** 事件使您能够在加载网页时检索特定文本。另外，您还可以在**自定义**对话框的新**网页浏览器**页面中自定义初始主页 URL。请参阅 <a href="https://github.com/Emurasoft/library/blob/main/macros/Bing.jsee" target="_blank">**Bing.jsee** 宏示例</a>以了解如何利用新功能。 {{pro}}
