...
- 添加了可视化隐形、控制 和/或代理项字符 的功能，这可能会导致潜在的安全问题。字符范围是可自定义的。此外，新版本可以选择在文件保存之前立即检查这些字符。
...
- 添加了 [字符范围 对话框](../dlg/properties/char_check/char_range/index) 到配置属性中的字符检查 页面上。
- 添加了使用 .editorconfig 选项到自定义 对话框的 [编辑 页面](../dlg/customize/edit/index) 让您可以选择是否要支持EditorConfig。
- 添加了确保每个文件末尾都有换行符 选项到 [保存详细信息 对话框](../dlg/properties/file/save_details/index) 中。
- 添加了警告字符 项目到配置属性中的 [显示 页面](../dlg/properties/display/index) 上的指定部分 列表框中。
...
- 添加了EI\_GET\_SUM 参数到 [EE\_INFO 消息](../plugin/message/ee_info) 中。

## 新增宏功能

- 改进了Sum.jsee 宏以在筛选时从总和中排除隐藏的行。
- 将eeColorInvalidCharacters 索引添加到DisplayList 的 [Item 属性](../macro/display_list/item) 集合中。
- 添加了[Sum](../macro/selection/sum) 和[Average](../macro/selection/average) 屬性到Selection 对象中。
