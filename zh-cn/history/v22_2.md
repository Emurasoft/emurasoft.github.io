...
- 添加了\\V 表达式到 [替换表达式语法](../howto/search/replacement_expression_syntax)，让您能把V8 JavaScript 引擎 用于替换表达式。
- [CommitList 插件](../howto/plugin/plugin_commit_list) 现在可以调用拉取、推送和提取命令。
- 添加了能筛选记录的功能到CommitList 中。
- 改进了崩溃报告。
...
- 添加了自动管理所有内存大小 复选框到自定义 对话框的 [高级 页面](../dlg/customize/advanced/index) 上。
- 添加了拉取 和推送 按钮以及提取 菜单项目到 [CommitList 插件](../howto/plugin/plugin_commit_list) 中。
- 添加了筛选记录 对话框到 [CommitList](../howto/plugin/plugin_commit_list) 中。
- 添加了总是丢弃过长的撤销信息来加速 选框到进度 对话框中（与自定义 对话框的编辑 页面中同名的选项相同）。
...
- 添加了nFlags 参数到 [EE\_SET\_STATUSW 消息](../plugin/message/ee_set_statusw) 和 [Editor\_SetStatusW 内联函数](../plugin/macro/editor_setstatusw) 中。

## 新增宏功能 <pro />

- 添加了eeMacroLangV8 和eeMacroSyncOnly 标志到Editor 对象的 [ExecuteMacro 方法](../macro/editor/editor_executemacro) 中。
