...
- 添加了同步 设置功能。默认设定不启用此功能。
- 添加了/max 选项到 [命令行选项](../howto/file/file_commandline) 中。

## 新增选项

- 添加了 [同步 页面](../dlg/customize/sync/index) 到自定义 对话框中。
- 添加了一个单元格中的最大换行数 文本框到自定义 对话框中的 [CSV 页面](../dlg/customize/csv/index) 上。
- 添加了每个文件的最大内存大小 文本框到自定义 对话框中的 [高级 页面](../dlg/customize/advanced/index) 上。
- 添加了App 主题 下列列表框到自定义 对话框的 [查看 页面](../dlg/customize/view/index) 上。
- 添加了停止如果匹配数达到 文本框到 [在文件中查找](../dlg/find_in_files/index) 以及 [在文件中替换](../dlg/replace_in_files/index) 对话框中。弃用了自定义 对话框中 [搜索 页面](../dlg/customize/search/index) 上的可替换的最大文件总数在保持开启的状态下 文本框。
...
- 添加了nLimit 字段到 [GREP\_INFO\_EX 结构](../plugin/structure/grep_info_ex) 中。

## 新增宏功能

- 添加了nExFlags 和nLimit 参数到Editor 对象的 [FindInFiles 方法](../macro/editor/editor_findinfiles) 以及 [ReplaceInFiles 方法](../macro/editor/editor_replaceinfiles) 中。
