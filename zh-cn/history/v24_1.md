...
- 添加了使用 [**OpenAI**](https://openai.com/) 完成单词或短语的功能。建议的文本会默认显示为灰色，您可以通过按 **Tab** 键接受。如果您只想确认建议的一部分，请按**向右**方向键（"→") ，按到您想要接受的字符数为止。按 **Esc** 键或其他键会取消建议。此外，如果您想要不同的建议，或在没有显示建议文本时强制 AI 进行预测，请按 Ctrl+空格（按住 Ctrl 键的同时按空格键）。该功能需要 [**OpenAI API 密钥**](https://platform.openai.com/api-keys) 才能使用。{{pro}}
