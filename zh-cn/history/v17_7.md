...
- 提高了各种排序 命令的速度。
- 文件比较后添加了生成报告文件的功能。 添加了eeCompare 配置和以竖线 分隔的 CSV 格式来查看比较报告。
- 将Python 语法更新到 Python 3.7.0b4。
...
- 添加了生成报告？ 页到比较 向导中。
- 添加了文件修改日期 (&e) 和文件修改时间 (&u) 宏到配置属性的 [打印 页面](../dlg/properties/print/index) 上的页眉/页脚 文本框中。
- 添加了颜色 下拉列表框到配置属性的 [高亮 (1) 页面](../dlg/properties/highlight1/index) 上，让你能指定与比较相关的颜色。
- 添加了使用临时文件的最小大小 文本框以及目前使用临时文件 消息到大文件控制器 自定义分栏中。
- 添加了忽略空单元格 复选框到 [删除/把重复行设为书签 对话框](../dlg/delete_duplicate_advanced/index) 中。
- 添加了比较 CSV 文档单元格中未加引号的字符串 复选框到自定义 对话框中的 [排序 页面](../dlg/customize/sort/index) 上。
...
- 添加了MANAGE\_DUPLICATES\_IGNORE\_EMPTY\_CELLS 标志到 [MANAGE\_DUPLICATES\_INFO 结构](../plugin/structure/manage_duplicates_info) 中。
- 添加了SORT\_UNQUOTE\_CELLS 标志到 [SORT\_INFO 结构](../plugin/structure/sort_info) 中。
...
- 添加了eeIgnoreEmptyCells 标志到Document 对象的 [DeleteDuplicates 方法](../macro/document/delete_duplicates) 中。
- 添加了eeSortUnquoteCells 标志到Document 对象的 [Sort 方法](../macro/document/sort) 中。
- 添加了 [Compare 方法](../macro/editor/compare) 到Editor 对象中。
