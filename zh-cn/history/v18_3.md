...
- 在筛选 工具栏中添加了「仅书签行」，「仅未标记书签的行」 和「匹配换行符」 按钮。
- 大大提高了删除换行符 以及验证含有嵌入式换行符的 CSV 文档的速度。
...
- [您现在可以对未保存的文档启用只读 模式。](https://www.emeditor.com/forums/topic/read-only/)
...
- 添加了随机排序 单选按钮到 [多列排序 对话框](../dlg/sort_multi/index) 中。
- 添加了二进制（十六进制视图） 下拉列表框到自定义 对话框的 [格式 页面](../dlg/customize/format/index) 上。
- 添加了仅书签行，仅未标记书签的行，匹配换行符，仅 CR，仅 LF，仅 CR+LF，其他 复选框到 [高级筛选 对话框](../dlg/advanced_filter/index) 中。

## 新增命令

- [随机排序](../cmd/edit/sort_random)
...
- 添加了SORT\_RANDOM 标志到 [Editor\_Sort 内联函数](../plugin/macro/editor_sort) 以及 [SORT\_INFO 结构](../plugin/structure/sort_info) 中。
- 添加了FLAG\_FIND\_BOOKMARKED\_ONLY，FLAG\_FIND\_CR\_ONLY，FLAG\_FIND\_CR\_LF，FLAG\_FIND\_LF\_ONLY，FLAG\_FIND\_NL\_OTHERS，FLAG\_FIND\_UNBOOKMARKED\_ONLY，以及FLAG\_FIND\_MATCH\_NL 标志到 [FILTER\_INFO\_EX 结构](../plugin/structure/filter_info_ex) 中。

## 新增宏功能

- 添加了R（随机排序）选项到 [Sort 方法](../macro/document/sort) 的strColumns 参数中。
- 添加了ExFlags 参数到 [Filter 方法](../macro/document/filter) 中。
