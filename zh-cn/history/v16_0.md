...
- 新增了添加引号到所有单元格上 按钮到CSV 转换器 上。
- 在自定义 对话框的 [窗口 页面](../dlg/customize/window/index) 上，添加了播放额外音效 复选框。
- 在自定义 对话框的高级 页面上最大内存大小 的默认值变为 90%。
- 新增了“把焦点设置到输出栏上” 复选框到 [外部工具属性](../dlg/tools/properties/index) 对话框中。
- 废弃了在自定义宏 对话框中的 [“选项” 页面](../dlg/macro_customize/options/index) 上的录制鼠标活动和键盘输入到其他应用程序中 复选框，录制所有鼠标移动 复选框，以及在插入休眠模式前的空闲时间 文本框。
...
- 添加了 [filters 属性](../macro/document/filters) 以及 [Untitled 属性](../macro/document/untitled) 到Document 对象中。
- 添加了eeFindFileAndLine，eeFindFileNamesOnly，eeFindLineOnly，eeFindMatchedOnly，和eeFindOutput 标志到Selection 对象的 [Find 方法](../macro/selection/selection_find) 中。
