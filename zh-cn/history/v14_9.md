...
- 新增的在 CSV 中删除嵌入式换行 命令让你能在 CSV 文档中删除嵌入式换行。
...
- 新的匹配 CSV 中的嵌入式换行 命令被添加到查找/替换 对话框中点击> 按钮显示的菜单项目中。
- 在查找 工具栏上添加了「匹配 CSV 中的嵌入式换行」 按钮。
- 在查找/替换/在文件中查找/在文件中替换 对话框中点击> 按钮显示的菜单项目中添加了固定值 选项。另外，自定义 被更名为上次使用的值。保存为默认值 被更名为自动保存，你能通过单击选项切换这些选项的启用/禁用状态。
- 在 [文件关联配置 对话框](../dlg/configuration_associations/index) 中添加了“用于‘在文件中查找’对话框中的文件类型” 的复选框。

## 新增命令

-[在 CSV 中删除嵌入式换行](../cmd/edit/remove_embedded_nl)
-[匹配 CSV 中的嵌入式换行 (查找工具栏)](../cmd/search/findbar_embedded_newlines)

## 宏的新增功能

-eeFindReplaceEmbeddedNL 标志被添加到Selection 对象 的[查找](../macro/selection/selection_find) 以及[替换](../macro/selection/selection_replace) 方法中。
