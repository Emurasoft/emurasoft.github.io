...
如果勾选了该复选框，EmEditor 将使用基于文件的操作进行编辑，并减少系统的内存使用量。如果还勾选了“从原始文件中读取未修改的行”复选框，则将从原始文件中读取未修改的行，从而提高打开文件的速度。否则，打开文件时，文件内容会被保存为临时文件，从而降低文件打开的速度。这两种情况都会减少系统的内存使用量。由于使用临时文件会降低运行速度，因此 EmEditor 仅当文件大小大于下面指定的大小时才使用临时文件。
