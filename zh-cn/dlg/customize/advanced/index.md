...
如果选中此框，EmEditor 将根据可用内存大小自动决定是否应在打开文件时使用临时文件。 启用此选项时，如果要打开的文件大小大于可用内存的大约1/4，EmEditor 将使用临时文件。 设置此选项允许 EmEditor 优化读取和写入的速度。 但是，如果你主要阅读大文件而不进行编辑，请禁用此选项并在 **使用临时文件的最小文件大小** 文本框中输入较小的数字，这样 EmEditor 能更快地读取大文件。
