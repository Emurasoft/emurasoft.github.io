...
| **固定值** | 当该选项被勾选时，对话框会把设为固定值的字符串作为查找文本框中的初始字符串。 |
| **通配符** | 勾选该选项后，您可以使用通配符来指定文件类型。 |
| **正则表达式** | 勾选该选项后，您可以使用正则表达式来指定文件类型。 |
...
| **显示文件名、行和匹配的行（截断过长的行）** | 包含所搜索字符串的文件名、行号以及整行内容将作为结果显示。过长的行将被截断。 |
...
|     |     |
| --- | --- |
| **添加链接的文件...** | 选择包含多个由换行符分隔的查找字符串的文件。这与**导入**类似，但链接文件仅包含查找字符串。链接文件使用的内存也较少，因为该文件在“查找”列表框中显示为一个条目。 |
