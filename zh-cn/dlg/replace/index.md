...
当该复选框被勾选时，搜索会在你输入字词到查找 下拉列表框中那一刻立即开始。
...
从当前位置向下搜索通过选择查找下一个 或替换。如果直到文档末尾都没有发现这个单词的话，它会从文档顶部继续搜索。
...
|到新 CSV 列 | 用替换字符串填充新的 CSV 列，而原始列保持不变。如果没有勾选使用正则表达式 选项，在替换为 中的文本要为空，那么新列会用匹配的查找项填充，不做任何改变。新的列会插入在原始列的右侧。 |
...
在替换 对话框与批处理替换 对话框之间切换。

下列控件仅在该对话框扩展为批处理替换 对话框时显示。
