...
- 在默认设定下，一个被插入的字符串能通过执行一次 [撤消 命令](../cmd/edit/edit_undo) 就被撤消。这个行为能被还原到之前的行为通过在 [自定义 对话框](../dlg/customize/index) 中 [高级 页面](../dlg/customize/advanced/index) 上取消勾选逐格撤消（需要重启 EmEditor ） 复选框。
...
-被另一个程序更改时 下拉列表框中的提示，自动重新载入，或保存锁定 选项允许 EmEditor 变更只读状态当文件的只读属性变化时。
