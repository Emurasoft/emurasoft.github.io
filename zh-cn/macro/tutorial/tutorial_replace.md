...
通常在 [Replace 方法](../selection/selection_replace)，与 [Find 方法](../selection/selection_find) 相同，不会终止执行一个宏当没有找到搜索字符串。但是，有一个例外。如果你用宏 菜单下的 [使用临时选项运行 命令](../../cmd/macros/macro_run_options)，并在弹出的 [宏临时选项 对话框](../../dlg/macro_temp_options/index) 中勾选了搜索失败即停止 复选框，那么宏就会被终止当没有找到搜索字符串时。详见教程中的 [查找一个字符串](tutorial_find)。
