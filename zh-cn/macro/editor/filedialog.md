...
| eeFileDialogFileMustExist | 指定用户只能输入已存在文件的文件名在 **文件名** 输入字段。如果指定了该标志而用户输入了一个无效的名称，对话框会显示一条警告消息。如果指定了该标志，eeFileDialogPathMustExist 也会被用到。 |
