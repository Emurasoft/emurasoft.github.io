...
### Anthropic
1. 登录 Claude Console，并访问 [API 密钥页面](https://platform.claude.com/settings/keys)。
2. 点击 **创建密钥**，并为该密钥添加名称。
3. 复制该 API 密钥。
...
- 在聊天面板底部的输入框中输入你的提示词。
- 单击 **上传文件** 按钮上传文件。  
  - 此功能仅适用于 Google 和 OpenAI。文件搜索会产生额外费用。  
  - 单击 **浏览**，通过文件浏览器对话框选择受支持的文件。  
  - 上传文件后，“与 AI 聊天”会同时创建文件资源和向量存储资源。之后即可在聊天过程中引用该文件。  
  - 已上传的文件会显示在输入框下方。单击文件可将其移除，或右键单击以查看更多选项。  
  - 引用已上传的文件通常比直接将文件内容复制粘贴到聊天中消耗更少的代币（token）。  
- 单击 **AI 模型名称** 以更改每个聊天的设置。  
  - 在此处所做的更改仅应用于当前聊天会话。要调整全局设置，请使用侧边栏中的 **设置** 按钮。  
- 单击 🔨（**工具**）按钮查看工具调用的设置。这些 EmEditor 工具允许 AI 读取或修改你的文档。  
  - 使用工具会产生额外的 token 费用。  
- 单击 🔌（**MCP**）按钮打开 MCP 连接器设置。MCP 连接器将在单独的章节中说明。
...
- 该对话框会显示与您的账户或组织关联的所有文件和向量存储。对于每个向量存储，还会显示其关联的聊天以及当前状态。
  - 已过期的向量存储不会产生存储费用。
- 您可以从您的账户或组织中选择并删除文件或向量存储。

## 导入与导出聊天
- 您可以将聊天对话导出到文件。该文件可以导入以恢复对话。
