...
用于 [EE\_EDIT\_COLUMN](../message/ee_edit_column) 消息。
...
> 您可以指定以下值的组合。必须指定 SORT\_ENABLED 对字符串进行排序，并与其他标志结合以指定排序行为。 必须指定 SORT\_DELETE\_DUPLICATE 才能删除重复的字符串。只有在 _nFlags_ 字段中指定了 COLUMN\_CONCAT 时，才使用此字段。
>
> | 值 | 含义 |
> | --- | --- |
