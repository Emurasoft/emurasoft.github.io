...
TheTab page allows you to customize settings related to the tabs.
...
|Truncate only when longer than specified width | Each tab is adjusted to its file name length, but truncated only when its length is longer than a width specified in theSpecified Width text box. |
|Fix to specified width | All tabs are same in length as specified in theSpecified Width text box. |
...
Specifies the width of each tab whenTruncate only when Longer than specified width orFixed to specified width is selected for theItem Width drop-down list box.
...
Highlights the active tab when theTab is selected for theStyle drop-down list box.

## Color button

Specifies the color to highlight the active tab when theHighlight Active Tab check box is set.
