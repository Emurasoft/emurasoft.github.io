...
This dialog box appears when the [Replace command](../../cmd/search/edit_replace) is selected. You can specify a string to find and a string to replace it with, as well as other options.
...
Searches words only. A word is defined as a string that begins and ends with any of these characters: A – Z, a – z, 0 – 9, or an underscore. Strings surrounded by full-width characters
...
Replaces within the selected text only.

## Wrap around checkbox

Searches down from the current position by choosing **Find Next** or **Replace**. If the word is not found by the end of the file, it will continue searching from the top of the file.

## Close when finished checkbox

Closes the dialog box when EmEditor finishes replacing.
