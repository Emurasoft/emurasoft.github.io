...
--|--|--|--
Bookmarks|Clear All Bookmarks in This Document|Ctrl+Shift+F2|Clears all bookmarks in the current document.
Bookmarks|Next Bookmark in This Group|F2|Goes to the next bookmark in this group.
Bookmarks|Previous Bookmark in This Group|Shift+F2|Goes to the previous bookmark in this group.
Bookmarks|Toggle Bookmark|Ctrl+F2|Toggles a bookmark on the current line.
CSV|Bottom Edge (Cell Selection Mode Only)|Ctrl+Down|Moves to the bottom edge of the current data region.
CSV|Bottom Edge Extend (Cell Selection Mode Only)|Ctrl+Shift+Down|Extends the selection to the bottom edge of the current data region.
CSV|Clear Contents (Cell Selection Mode Only)|Delete|Clear the contents of the selection in the CSV document.
CSV|Edit Cell (Cell Selection Mode Only)|F2|Edits the current cell in the CSV document.
CSV|Left Edge (Cell Selection Mode Only)|Ctrl+Left|Moves to the left edge of the current data region.
CSV|Left Edge Extend (Cell Selection Mode Only)|Ctrl+Shift+Left|Extends the selection to the left edge of the current data region.
CSV|Next Cell|Alt+Shift+Right|Jumps to the next cell in CSV, TSV, or DSV mode.
CSV|Previous Cell|Alt+Shift+Left|Jumps to the previous cell in CSV, TSV, or DSV mode.
CSV|Right Edge (Cell Selection Mode Only)|Ctrl+Right|Moves to the right edge of the current data region.
CSV|Right Edge Extend (Cell Selection Mode Only)|Ctrl+Shift+Right|Extends the selection to the right edge of the current data region.
CSV|Top Edge (Cell Selection Mode Only)|Ctrl+Up|Moves to the top edge of the current data region.
CSV|Top Edge Extend (Cell Selection Mode Only)|Ctrl+Shift+Up|Extends the selection to the top edge of the current data region.
Compare|Next Change|F7|Jumps to the next change.
Compare|Next Changed Character|Ctrl+F7|Jumps to the next changed character.
Compare|Previous Change|Shift+F7|Jumps to the previous change.
Compare|Previous Changed Character|Ctrl+Shift+F7|Jumps to the previous changed character.
Compare|Rescan|F5|Rescans the compared documents and refreshes the results.
Convert|Decrease Line Indent|Shift+Tab|Decreases the line indent in the selection.
...
Edit|Char Left|Left|Moves the cursor one character to the left.
Edit|Char Left Extend|Shift+Left|Extends the selection one character to the left.
Edit|Char Left Extend (Vertical Mode)|Alt+Ctrl+Left|Extends the vertical selection one character to the left.
Edit|Char Right|Right|Moves the cursor one character to the right.
Edit|Char Right Extend|Shift+Right|Extends the selection one character to the right.
Edit|Char Right Extend (Vertical Mode)|Alt+Ctrl+Right|Extends the vertical selection one character to the right.
Edit|Copy|Ctrl+C, Ctrl+Insert|Copies the selection or the current line and pastes it to the Clipboard.
Edit|Copy in Quotes and Deselect|Ctrl+Shift+Q|Copies the selection in quotes, pastes it to the Clipboard and deselects the text.
Edit|Cut|Shift+Delete, Ctrl+X|Cuts the selection if selected or cuts the current line and moves it to the Clipboard.
Edit|Cut Lines|Ctrl+L|Cuts the selected lines or current line and moves it to the Clipboard.
Edit|Delete|Shift+Backspace, Delete|Deletes the selection, or deletes one character at the right of the cursor.
Edit|Delete Left Character|Backspace|Deletes the selection, or deletes one character at the left of the cursor.
...
Edit|Select All|Ctrl+A|Selects all.
...
Macros|Run Macro|Ctrl+Shift+P|Runs macro.
