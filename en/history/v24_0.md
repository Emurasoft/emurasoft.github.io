...
- Added the **Edit in EmEditor** command to the Windows 11 style context menu of Windows Explorer (64-bit Desktop Installer version only). There is currently a known issue where the EmEditor icon opened by this command will appear transparent when viewed on the taskbar, and will appear as a separate icon from other EmEditor windows. {{profree}}
