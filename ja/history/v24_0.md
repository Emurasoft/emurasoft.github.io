...
- Windows エクスプローラの Windows 11 スタイルのコンテキスト メニューに、[EmEditorで編集] コマンドを追加しました (64ビット デスクトップ インストーラー版のみ)。現在のところ、タスク バーを見ると、このコマンドにより開いた EmEditor のアイコンは透明になって表示されてしまい、他の EmEditor ウィンドウとは別のアイコンとして表示されてしまう既知の不具合があります。 {{profree}}
