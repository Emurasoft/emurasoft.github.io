...
文字列が見つかった場合は 1 を返します。見つからない場合は 0 を返します。ただし、FLAG\_FIND\_COUNT、FLAG\_FIND\_BOOKMARK、FLAG\_FIND\_SELECT\_ALL、または FLAG\_FIND\_EXTRACT フラグが指定されている場合、戻り値は文書内で一致した文字列の数になります。しかし、検索文字列がカーソル位置より指定した方向に見つからない場合は、残りの文書で一致している場合でも、戻り値は 0 になります。
