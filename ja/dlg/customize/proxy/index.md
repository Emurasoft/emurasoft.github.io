...
[プロキシ] ページでは、プロキシ接続に関連する設定をカスタマイズできます。EmEditor は、このページのプロキシ設定を使用して、`support.emeditor.com` に HTTP および HTTPS で接続します。また、AI 機能を使用する場合は、`api.openai.com` に接続するためにプロキシの設定を使用します。
...
このボタンをクリックして、上記で指定した設定を使用して `support.emeditor.com` への接続をテストします。接続が成功すると、このボタンの左側に「✓ 接続成功！」と表示されます。
