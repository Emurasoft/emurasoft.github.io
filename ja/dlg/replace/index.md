...
このダイアログ ボックスは、 [\[置換\] コマンド](../../cmd/search/edit_replace) を実行すると表示されます。ここで指定する文字列とオプションで検索します。
...
このボタンをクリックすると、検索文字列や各オプションをそのまま保存し、現在のダイアログ ボックスを閉じて、 [\[検索\] ダイアログ ボックス](../find/index) に切り替わります。
